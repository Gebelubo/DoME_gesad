...
class AIEngine(DAO):
        ...
        if aggregation_strategy is None:
            for i in range(len(tokens) - 1, -1, -1):
                if i > 0 and tokens[i]['word'].startswith('##'):
                    ...
                    if not tokens[i - 1]['word'] in ANALYTICS[0]:  # do not merge if the word is an analytic keyword
                        if not tokens[i - 1]['word'] in ANALYTICS[2]:
                            if not tokens[i - 1]['word'] in ANALYTICS[1]:
                                if not tokens[i - 1]['word'] in ANALYTICS[3]:
    ...
    class __MsgParser:
        ...
        def __get_entity_class_from_msg(self) -> str:
            ...
            for token in self.tokens:
                if (token['entity'] == 'NOUN' and  # only nouns
                    ...
                    if list_util.compare_item_list(token['word'], ANALYTICS):
                        # this prevents an analytic keyword be interpreted as an entity
                        continue
                    if len(candidates) == 0:
            ...
            if len(strong_candidates) == 1:
                ...
                question = "Chatbot context: the user is requesting an " + str(
                    self.intent) + " operation. User message: '" \
        ...
        def __get_attributes_from_msg(self) -> tuple:
            ...
            j = 0
            analytic_opr = {'request': None, 'operation': None}
            while j < len(self.tokens):
                ...
                while j < len(self.tokens) and token_j is None:
                    if self.tokens[j]['entity'] is not None:  # prevents token errors
                        if list_util.compare_item_list(self.tokens[j]['word'], ANALYTICS):
                            analytic_opr['operation'] = self.tokens[j]['word']
                            j += 1
                            ...
                            j += 1
                        elif self.tokens[j]['word'] == self.entity_class:
                ...
                if token_j:
                    if analytic_opr['operation'] is not None:
                        if 'entity' not in analytic_opr:
                            analytic_opr['entity'] = "None"
                        analytic_opr['request'] = token_j['word']
                        return analytic_opr, where_clause_attributes
